...
fn dump(fware: String) {
    ...
    for _i in 0..num_ops {
        ...
        if alignment_rem != 0 {
            ...
            cursor
                .seek(SeekFrom::Current(alignment_padding as i64))
                .unwrap();
        }
        ...
        if op_type == update_file {
            ...
            if b {
                ...
                assert!(
                    env::current_dir().unwrap() == dump_path,
                    "{} != {}",
...
fn decrypt(fware: String) -> (u32, Vec<u8>) {
    let pubkey_n_raw = Bytes::from_static(&[
        ,
    ]);
    let pubkey_n = num_bigint::BigInt::from_bytes_be(Sign::Plus, &pubkey_n_raw);
    let pubkey_e = num_bigint::BigInt::from_bytes_be(
        Sign::Plus,
    ...
    assert!(
        u32::from_le_bytes(magic) == 0x50cd50cd,
        "Not valid firmware magic {}",
        u32::from_le_bytes(magic)
    );
    assert!(
        u32::from_le_bytes(product) == 0x010000a5,
        "Not valid product {}",
        u32::from_le_bytes(product)
    );
    assert!(
        u32::from_le_bytes(hdr_size) >= 16,
        "Header size less than 16 {}",
        u32::from_le_bytes(hdr_size)
    );
    ...
    let raw_params_blob = encrypted_params
        .modpow(&pubkey_e, &pubkey_n)
        .to_str_radix(16);
    let raw_params =
        hex::decode(&raw_params_blob[raw_params_blob.len() - (88 * 2)..raw_params_blob.len()])
            .unwrap();
    assert!(88 == raw_params.len());
    let pam_after = Instant::now();
    println!(
        "Parameter Decryption took {:?}",
        pam_after.duration_since(pam_before)
    );
    ...
    let enc_payload_sz = fsize as usize
        - (4 as usize
            + u32::from_le_bytes(hdr_size) as usize
            + u32::from_be_bytes(params_size) as usize)
        - pubkey_n_raw.len() as usize;
    let mut encrypted_payload = vec![0u8; enc_payload_sz as usize];
    f.read(&mut encrypted_payload).unwrap();
    assert!(
        encrypted_payload.len() == enc_payload_sz,
        "actual {} != calculated {}",
    ...
    assert!(
        raw_signature.len() == pubkey_n_raw.len(),
        "{}/{}/256 {}",
        raw_signature.len(),
        pubkey_n_raw.len(),
        fsize - encrypted_payload.len() as u64
    );
    assert!(
        32 == aes256_key.len(),
        "invalid length for AES256 key: {}",
        aes256_key.len()
    );
    let cipher = Cipher::new_256(aes256_key.try_into().unwrap());
    let before_dec = Instant::now();
    let payload = cipher
        .cbc_decrypt(aes256_iv.try_into().unwrap(), &encrypted_payload[..])
        .to_vec();
    let after_dec = Instant::now();
    println!(
        "Payload Decryption took {:?}",
        after_dec.duration_since(before_dec)
    );
...
fn verify_signature(data: &[u8], encrypted_sig: &[u8], key: &[u8], e: BigInt, n: BigInt) {
    let pkcs1_rsa_sha256_id = Bytes::from_static(&[
    ...
    assert!(
        ...
        "expected/actual signature mismatch {} != {}",
        expected_sig,
        actual_sig
    );
}
